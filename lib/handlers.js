   ...
   if (firstName && lastName && phone && password && tosAgreement){
      ...
      _data.read('users',phone,(err,data) => {
          if (err) {
              ...
              callback(403,{'Error':' A user with that phone number arleady exist '});
          }
       ...
       _data.read('users',phone,(err,userData)=>{
           if(!err && userData){
               ...
               if (hashedPassword == userData.hashedPassword){
                  ...
                  const expires = Date.now() + 24000 * 60 * 60;
    ...
    if (id) {
       _data.read('tokens',id,(err,tokenData)=>{
           if (!err && tokenData){
             ...
             callback(400,{'Error':"Token associated with User does not exist"}) 
           }
       });
        ...
        _data.read('tokens',id,(err,tokenData)=>{
            if(!err && tokenData){
                if(tokenData.expires > Date.now()){
                    tokenData.expires = Date.now() + 24000*60*60
